...
import { getTeamSpeakChannels } from "@/lib/teamspeak-admin";
...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await getTeamSpeakChannels(server));
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not load TeamSpeak channels." },
      { status: 502 },
    );
  }
}
